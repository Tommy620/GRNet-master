...
# based on https://github.com/AlexWang0214/MRCNet/blob/master/train_with_sacred.py
...
from utils import (mat2xyzrpy, merge_inputs, overlay_imgs, quat2mat,
                   ...
                   tvector2mat,point_cloud_washing)
...
def config():
    ...
    loss = 'multi_dim_loss'
    max_t = 0.5 #  0.5,  0.2, 0.1 modify this hyper-param so you can train model with smaller range —— for perturbation forming on translation
    max_r = 5.0 # 5.0,  2.0,  1.0° modify this hyper-param so you can train model with smaller range —— for perturbation forming on rotation
    batch_size = 10
    num_worker = 8
    network = 'Res_f1'
    optimizer = 'adam'
    resume = False
    weights = None
    rescale_rot = 1.0  #param for weighing regression loss
    rescale_transl = 1.0 #param for weighing regression loss
    precision = "O0"
    norm = 'bn'
    dropout = 0.0
    max_depth = 80.
...
# CCN training
@ex.capture
def train(model, optimizer, rgb_img, refl_img, target_transl, target_rot, loss_fn, point_clouds, loss,rgb,cam_calib):

    model.train()

    point_clouds = point_cloud_washing(target_transl, target_rot, point_clouds,cam_calib)
    ...
    losses = loss_fn(point_clouds, target_transl, target_rot, transl_err, rot_err,cam_calib)
...
def main(_config, _run, seed):
    ...
    img_shape = (384, 1280)
    input_size = (256, 512) #this is the size for img and depth map entering model, seperately to two branches
    ...
    if _config['loss'] == 'multi_dim_loss':
        loss_fn = Multi_dim_Loss(_config['rescale_transl'], _config['rescale_rot'], _config['weight_point_cloud'])
    else:
    ...
    if _config['optimizer'] == 'adam':
        optimizer = optim.Adam(model.parameters(), lr=_config['BASE_LEARNING_RATE'], weight_decay=5e-6)
        ...
        optimizer = optim.SGD(model.parameters(), lr=_config['BASE_LEARNING_RATE'], momentum=0.9,
    ...
    if _config['weights'] is not None and _config['resume']:
        ...
        optimizer.load_state_dict(opt_state_dict, strict=False)
        if starting_epoch != 0:
    ...
    for epoch in range(starting_epoch, _config['epochs'] + 1):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            for idx in range(len(sample['rgb'])):
                ...
                depth_gt, uv = lidar_project_depth(pc_lidar, sample['calib'][idx], real_shape)
                ...
                pc_rotated = rotate_back(sample['point_cloud'][idx], RT)

                if _config['max_depth'] < 80.:
                    pc_rotated = pc_rotated[:, pc_rotated[0, :] < _config['max_depth']].clone()

                depth_img, uv = lidar_project_depth(pc_rotated, sample['calib'][idx], real_shape)
            ...
            loss, R_predicted,  T_predicted = train(model, optimizer, rgb_input, lidar_input,
                                                    ...
                                                    loss_fn, sample['point_cloud'], _config['loss'],sample['rgb'],sample['calib'])
        ...
        for batch_idx, sample in enumerate(ValImgLoader):
            ...
            trloss+=loss['transl_loss'].item() * len(sample['rgb'])
            rotloss+=loss['rot_loss'].item() * len(sample['rgb'])
            val_iter += 1

        print("------------------------------------")
        print('total val loss = %.3f' % (total_val_loss / len(dataset_val)))
        print(f'total traslation loss: {trloss / len(dataset_val)} ')
        print(f'total rotation loss: {rotloss / len(dataset_val)} ')
        print(f'total mixedQ loss: {2*(total_val_loss-trloss-rotloss*0.5) / len(dataset_val)} ')
