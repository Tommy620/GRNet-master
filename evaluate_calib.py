...
def main(_config, seed):
    ...
    total_time = 0
    total_net_time = 0
    total_time_per_model = [0.0] * len(weights)
    num_iters = 0
    ...
    for batch_idx, sample in enumerate(tqdm(TestImgLoader)):
        ...
        if _config['save_image']:
            ...
            cv2.imwrite(os.path.join(depth_path, sample['rgb_name'][0]), depth_img[0, :720, :1280])
        ...
        with torch.no_grad():
            for iteration in range(start, len(weights)):
                ...
                torch.cuda.synchronize()
                per_model_time.append(time.time() - t_model)
                net_time += per_model_time[-1]


                if batch_idx == 0 and iteration == 0:

                    inputs = (rgb_resize, lidar_resize)
        ...
        tqdm.write(f"[frame {batch_idx}] {len(per_model_time)} models forward: {net_time * 1000:.2f} ms "
                   f"({split_str} ms) | whole frame (forward+reproject+metrics): {run_time * 1000:.2f} ms"
                   + ("  [warm-up, excluded from average]" if is_warmup else ""))

        if not is_warmup:
            num_iters += 1
            total_time += run_time
            total_net_time += net_time
            for k, t in enumerate(per_model_time):
                total_time_per_model[k] += t
    ...
    if num_iters > 0:
        ...
        for k in range(len(weights)):
            print(f"  model {k}: {total_time_per_model[k] / num_iters * 1000.0:.2f} ms/frame")
        print(f"  {len(weights)} models pure forward: {net_inference_time:.2f} ms/frame, "
              f"FPS: {1000.0 / net_inference_time:.2f}")
        print(f"  whole frame (forward+reproject+metrics): {inference_time:.2f} ms/frame, "
              f"FPS: {1000.0 / inference_time:.2f}")
    ...
    for i in range(len(weights) + 1):
        ...
        with open(os.path.join(_config['output'], 'results.txt'),
            ...
            f.write(f"Iteration {i}: \tMean Translation X Error: {errors_t2[i][-1][0].mean():.4f} cm "
                    f"     Median Translation X Error: {errors_t2[i][0].median():.4f} cm "
                    f"     Std. Translation X Error: {errors_t2[i][0].std():.4f} cm \n")
            f.write(f"Iteration {i}: \tMean Translation Y Error: {errors_t2[i][-1][1].mean():.4f} cm "
                    f"     Median Translation Y Error: {errors_t2[i][1].median():.4f} cm "
                    f"     Std. Translation Y Error: {errors_t2[i][1].std():.4f} cm \n")
            f.write(f"Iteration {i}: \tMean Translation Z Error: {errors_t2[i][-1][2].mean():.4f} cm "
                    f"     Median Translation Z Error: {errors_t2[i][2].median():.4f} cm "
                    f"     Std. Translation Z Error: {errors_t2[i][2].std():.4f} cm \n\n")

            # rotation rpy
            f.write("Rotation Error RPY:\n")
            f.write(f"Iteration {i}: \tMean Rotation Roll Error: {errors_rpy[i][-1][0].mean(): .4f} °"
                    f"     Median Rotation Roll Error: {errors_rpy[i][0].median():.4f} °"
                    f"     Std. Rotation Roll Error: {errors_rpy[i][0].std():.4f} °\n")
            f.write(f"Iteration {i}: \tMean Rotation Pitch Error: {errors_rpy[i][-1][1].mean(): .4f} °"
                    f"     Median Rotation Pitch Error: {errors_rpy[i][1].median():.4f} °"
                    f"     Std. Rotation Pitch Error: {errors_rpy[i][1].std():.4f} °\n")
            f.write(f"Iteration {i}: \tMean Rotation Yaw Error: {errors_rpy[i][-1][2].mean(): .4f} °"
                    f"     Median Rotation Yaw Error: {errors_rpy[i][2].median():.4f} °"
                    f"     Std. Rotation Yaw Error: {errors_rpy[i][2].std():.4f} °\n\n\n")
